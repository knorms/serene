...
var temp = []
for(var i=0; i<svm_input_train.length; i++){
        temp.push(svm_input_train[i]);
}

svm_input_train = temp;
temp = [];
...
for(var i=0; i<svm_input_test.length; i++){
        temp.push(svm_input_test[i]);
}

svm_input_test = temp;

// test model on 80% train, 20% val data
var clf_test = new svm.CSVC({probability: true, kernelType: 'POLY', degree:[2], normalize:false});
...
var clf = new svm.CSVC({probability: true, kernelType: 'POLY', degree:[2], normalize:false});
...
io.on('connection', function(socket){
  ...
  socket.on('init', function(msg){
    ...
    if(featurePoints != null) {
        ...
        //calculateDifference, reshape the way MATLAB does
        var diff = []
    ...
    //Return the probability distribution to the client
