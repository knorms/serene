// SOURCE: https://github.com/machow/procrustesjs/blob/master/procrustes.coffee
// Translated to javascript using: http://coffeescript.org/#coffeescript-2 
var express = require("express");
var app = require('express')();
var http = require('http').Server(app);
var io = require('socket.io')(http);
var ImageRater = require('./js/procrustes.js');
...
fs = require('fs');
var svm_input
svm_input = fs.readFileSync('./clm_traindata4node.txt','utf8');
svm_input = JSON.parse(svm_input)

svm_val = fs.readFileSync('./clm_valdata4node.txt','utf8');
svm_val = JSON.parse(svm_val)

var correct_labels=[];
svm_val.forEach(function(ex){
    correct_labels.push(ex[1]);
});

var clf = new svm.CSVC({probability: true});

var predicted_labels=[];
var i=0;
num_correct = 0;
num_total = 0;
clf.train(svm_input).done(function () {
    // predict things
    svm_val.forEach(function(ex){
        var prediction = clf.predictSync(ex[0]);
        predicted_labels.push(prediction);
        console.log("Predicted: "+predicted_labels[i]+", Correct: "+correct_labels[i]);
    ...
    console.log("num total: "+num_total);
    console.log("num correct: "+num_correct);
    console.log("Accuracy: "+ (num_correct/num_total));
});
...
io.on('connection', function(socket){
  ...
  socket.on('featurePoints', function(msg){
      //console.log('message: ' + msg);
      console.log(msg[0]);
      var featurePoints = msg[0];

	  //Do some stuff with the feature points like run through svm
	  if(featurePoints != null) {
	    ...
	    var faceAfter = applyProcrustes(featurePoints, procrustesParams);
	    console.log("avgNeutral");
	    console.log(avgNeutral);
	    console.log("faceAfter");
	    console.log(faceAfter);
	   };

        var diff = []
        for (var i = 0; i < curFace.length; i++) {
           diff[i] = curFace[i][0] - avgNeutral[i][0]; //diff x
           diff[i + curFace.length] = curFace[i][1] - avgNeutral[i][1]; //diff y
        }
        prediction = clf.predictSync(diff);

      //Return the feature Points to the client
      io.emit('featurePointsResponse', msg[0]);
...
//function taken from clmtrackr.js
